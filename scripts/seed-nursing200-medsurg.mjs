// ============================================================
// Seed the Medical-Surgical Nursing subject into the DB
// `questions` table for Nursing 200-Level. The subject had
// question data in the client bank file (`200level questions.json`)
// but was removed from the UI earlier when it was missing in the DB
// (one of the "phantom" subjects). This migrates ONLY those rows
// using the same normalizer as seed-questions.mjs / seed-nursing200-missing.mjs.
// Idempotent: fixed id prefix + upsert on id.
// ============================================================
...
const SUBJECT_ID = 'Medical-Surgical Nursing';
...
const rows = [];
for (const q of bank) {
  if (q.subject !== SUBJECT_ID) continue;
