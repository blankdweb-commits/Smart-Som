...
const normalizeQuestion = (q) => {
  ...
  try {
    ...
    if (Array.isArray(q.options)) {
      // Format A or standard array
      options = [...q.options];
    } else if (q.options && typeof q.options === 'object') {
      // Format B: options: { A: "...", B: "..." }
      options = Object.values(q.options);
    } else if (q.option_a || q.option_A) {
      // Format C: option_a, option_b...
      options = [
    ...
    if (options.length === 0) {
      ...
      options = [fallbackAns, "Option B", "Option C", "Option D"];
    }
    ...
    if (typeof ca === 'number' && ca >= 0 && ca < options.length) {
      // Index based (Format A)
      correctAnswerText = options[ca];
    } else if (typeof ca === 'string') {
      if (ca.length === 1 && /^[A-D]$/i.test(ca)) {
        // Key based (Format B/C: "A", "B"...)
    ...
    console.error("Normalization Error:", error, q);
    return null;
  }
};
...
const Quiz = () => {
  ...
  const handleConfirmAnswer = (opt = selectedOption) => {
    ...
    if (correct) {
      if ((score + 1) % 4 === 0) {
        setToast({ message: MOTIVATIONAL_MESSAGES[Math.floor(Math.random() * MOTIVATIONAL_MESSAGES.length)], type: 'info' });
      }
      ...
      <div className="flex-1 flex flex-col px-4 -mt-10 z-10 max-w-3xl mx-auto w-full overflow-y-auto no-scrollbar pb-32">
         ...
         <div className={`flex flex-col gap-4 ${eliminatedOptions.length > 0 ? "min-h-[300px]" : ""}`}>\n            {/* Expandable Options Layout */}
            {currentQ.options.map((option, idx) => {
               ...
               return (
                  <motion.div key={idx} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} layout transition={{ duration: isLowPowerMode ? 0.1 : 0.3 }}>
...
const OptionButton = ({ label, index, state, pollValue, onClick, disabled, dark, isSpeed }) => {
  ...
  return (
    <button
      ...
      onClick={() => { if(label.length > 80 && !isExpanded) setIsExpanded(true); else onClick(); }}
      className={`relative flex items-center justify-between w-full h-full p-6 rounded-[2rem] border-2 transition-all duration-300 ${baseStyles} active:scale-[0.98] shadow-lg mb-4`}
    >
      <div className="flex flex-col items-start pr-8">
        <p className={`font-bold text-left text-sm sm:text-base ${!isExpanded && label.length > 100 ? 'line-clamp-2' : ''}`}>{label}</p>
        {pollValue !== undefined && (
          <div className="mt-2 w-32">
            <div className="h-1 bg-white/10 rounded-full overflow-hidden">
              <motion.div initial={{width:0}} animate={{width:`${pollValue}%`}} className="h-full bg-blue-500" />
            </div>
            <p className="text-[10px] font-black mt-1 opacity-60">{pollValue}% choosing this</p>
          </div>
        )}
      </div>
...
const Shuffle = () => (<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M16 3h5v5" /><path d="M4 20L21 3" /><path d="M21 16v5h-5" /><path d="M15 15l6 6" /><path d="M4 4l5 5" /></svg>);
