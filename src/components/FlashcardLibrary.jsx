...
const FlashcardLibrary = ({ initialCategory = 'Academic' }) => {
  const { allFlashcards, exams, userProfile, addFlashcard, updateFlashcard, deleteFlashcard, incrementCardsStudied, updateCardProgress, importFlashcards } = useAppContext();
  ...
  useEffect(() => {
    ...
    if (subjectParam) {
      ...
      const card = allFlashcards.find(c => c.subject === subjectParam);
      if (card) {
  ...
  const categoryCards = useMemo(() => {
    ...
    if (initialCategory !== 'Academic') {
      return allFlashcards.filter(c => c.category === initialCategory);
    }
  ...
  }, [allFlashcards, initialCategory, currentProgram]);
